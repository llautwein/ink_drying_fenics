...
class Config:
    ...
    def __init__(self):
        ...
        self.num_steps = 50  # number of time steps
        self.dt = self.T / self.num_steps
        self.mu = 0  # moisture parameter
        ...
        self.phi_options = {
            ...
            "gaussian": Expression("2 * exp((-pow(x[0]-L/2,2) - pow(x[1]-L/2,2))/10)-1", L=self.L, degree=2),
        ...
        self.n_options = {
            "constant": Constant(0),
            "gaussian": Expression("2 * exp((-pow(x[0]-L/2,2) - pow(x[1]-L/2,2))/10)", L=self.L, degree=2),
